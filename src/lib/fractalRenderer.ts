...
void main() {
    ...
    vec2 scaled_pix_pos = pix_pos * u_resolution.xy / 1000.0 + vec2(0.0, 0.5);
...
void main() {
    ...
    if(distance_to_set <= epsilon) {
        ...
        // color = normal;
        color = vec4(color_surface(pos, normal, (marching_res.iter)), 0.9);
    }
    fragColor = color;
}
    `;
    ...
    function render(): void {
        if (isRendering) {
            ...
            // gl.uniform2f(resolutionUniform, canvas.width, canvas.height*1.7);
            gl.uniform2f(resolutionUniform, 1700, 1700); //camera size in fractal space
