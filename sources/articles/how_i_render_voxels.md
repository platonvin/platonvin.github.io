...
### Key structures
<!-- <small> they are actual struct's! </small> -->

* `Material` -- represent type of voxel by its properties, such as color, roughness, emittance, etc.

* `Material palette` -- Array of materials. Currently, limited by 255 (256, but first one is never accessed and represent "air")

* `Voxel` -- Index into a material palette (so, data of voxel is its material). Represented by single u8. 0 means "air" - empty voxels

* `Model` -- Free-form, self-aligned (so not world-aligned) voxel mesh of any size (up to 255x255x255 to fit in u8).
  Each model's voxels live in its own 3D image.

* `Block` -- Grid-aligned voxel mesh, exactly 16x16x16 in size. Blocks voxel data lives in block palette

* `Block palette` -- Large 3D image, stores (many) 16^3 blocks
...
* `World` -- 3D grid of block "pointers" (indices into Block palette).

* `Radiance field` -- 3D grid of lighting probes, one per block, representing corresponding (per-block) ligting
...
* CPU preparation: Regular CPU GAPI-state work. We also sort requests by depth (for Vulkan) / state (for WGPU), update per-frame GPU data and figure out which radiance probes need to be updated this frame (any block with a non-empty neighbor) and upload their positions to the GPU
...
_Especially if you target lower end of the spectrum - Integrated GPUs, which specifically perform worse in temporal accumulation and love subpass optimizations._
